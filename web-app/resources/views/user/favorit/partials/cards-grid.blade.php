    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-6" 
        ...
        <template x-for="cafe in filteredCafes" :key="cafe.id">
            <div @click="selectedCafe = cafe" 
                 class="group relative block aspect-[4/5] rounded-2xl sm:rounded-[2rem] overflow-hidden shadow-xl hover:shadow-[#B87C39]/20 transition-all duration-500 bg-gray-900 cursor-pointer transform hover:-translate-y-1">
                ...
                <div class="absolute top-3 left-3 sm:top-5 sm:left-5 z-20">
                    <div class="bg-black/40 backdrop-blur-md border border-white/20 text-white px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl flex items-center gap-1.5 sm:gap-2">
                        <svg viewBox="0 0 24 24" class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#B87C39] fill-none stroke-current" stroke-width="2"><path d="M10 2v2M14 2v2M16 8a1 1 0 0 1 1 1v8a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4V9a1 1 0 0 1 1-1h14a4 4 0 1 1 0 8h-1M6 2v2"/></svg>
                        <span class="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest" x-text="cafe.category"></span>
                    </div>
                </div>

                <div class="absolute top-3 right-3 sm:top-5 sm:right-5 z-20 flex items-center gap-1 sm:gap-1.5" @click.stop="">
                    <button @click.prevent="toggleBookmark(cafe)" 
                            class="w-7.5 h-7.5 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center transition-all duration-300 group cursor-pointer focus:outline-none"
                        ...
                        <svg viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
                             :fill="cafe.bookmarked ? 'currentColor' : 'none'" class="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5 transition-transform duration-300 text-current">
                    ...
                    <button @click.prevent="toggleBlacklist(cafe)" 
                        ...
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5 transition-transform duration-300 text-current">
                ...
                <div class="absolute bottom-3.5 left-3.5 right-3.5 sm:bottom-6 sm:left-6 sm:right-6">
                    <div class="mb-2 sm:mb-3">
                        <h3 class="text-sm sm:text-base md:text-xl font-black text-white leading-tight mb-0.5 sm:mb-1 tracking-tight line-clamp-1" x-text="cafe.name"></h3>
                        <div class="flex items-center gap-1.5 sm:gap-2 text-white/70 text-[9px] sm:text-[10px] font-medium">
                            <svg viewBox="0 0 24 24" class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#B87C39] fill-none stroke-current" stroke-width="2"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>
                            <span class="truncate" x-text="cafe.location"></span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2 sm:pt-3 border-t border-white/10">
                        <div class="flex flex-col">
                            <div class="flex items-center gap-0.5 sm:gap-1 mb-0.5">
                                <span class="text-xs sm:text-sm md:text-base font-black text-white" x-text="cafe.rating"></span>
                                <div class="flex gap-0.5">
                                    <svg viewBox="0 0 24 24" fill="currentColor" class="w-2 h-2 sm:w-2.5 sm:h-2.5 text-amber-400"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                                </div>
                            </div>
                            <span class="text-[7px] sm:text-[8px] font-bold text-white/40 uppercase tracking-widest" x-text="cafe.ratingLabel"></span>
                        </div>
                        <div class="text-right">
                            <span class="block text-xs sm:text-sm md:text-base font-black text-white" x-text="cafe.priceLabel"></span>

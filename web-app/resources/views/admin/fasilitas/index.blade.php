...
@section('breadcrumb')
    <x-admin.breadcrumb :links="[['label' => 'Fasilitas Kafe']]" />
@endsection
...
<div class="space-y-6 pb-12" x-data="fasilitasPage()" x-init="init()">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <form method="GET" action="{{ route('admin.fasilitas.index') }}" class="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="relative w-full md:max-w-sm">
                <i data-lucide="search" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400"></i>
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari fasilitas" class="w-full rounded-lg border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm font-medium text-gray-800 outline-none transition focus:border-[#B87C39] focus:ring-4 focus:ring-[#B87C39]/10">
            </div>
                ...
                <table class="w-full min-w-[680px] text-left">
                    <thead class="bg-[#6E4A22] text-white">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-xs font-bold uppercase tracking-wide">Nama Fasilitas</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($fasilitas as $item)
                            <tr class="transition hover:bg-[#B87C39]/5">
                                <td class="px-5 py-4 text-center text-sm font-bold text-gray-500">{{ $fasilitas->firstItem() + $loop->index }}</td>
                                <td class="px-5 py-4 text-sm font-bold text-gray-900">{{ $item->nama_fasilitas }}</td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <button type="button" @click="openEdit({ id: {{ $item->id_fasilitas }}, nama: @js($item->nama_fasilitas) })" class="inline-flex items-center gap-1.5 rounded-lg border border-[#B87C39]/30 bg-white px-3 py-2 text-xs font-bold text-[#6E4A22] transition hover:border-[#B87C39] hover:bg-[#B87C39] hover:text-white">
                                            <i data-lucide="pencil" class="h-3.5 w-3.5"></i>
                                            Edit
                                        </button>
                                        <button type="button" @click="confirmDelete({{ $item->id_fasilitas }}, @js($item->nama_fasilitas))" class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 py-2 text-xs font-bold text-red-600 transition hover:bg-red-600 hover:text-white">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-5 py-10 text-center text-sm font-semibold text-gray-500">Belum ada data fasilitas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="mt-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex items-center gap-1">
                ...
                @else
                    <a href="{{ $fasilitas->previousPageUrl() }}" class="px-3 py-1.5 rounded-lg text-sm font-bold text-[#B87C39] hover:bg-[#B87C39]/10 transition">&lsaquo;</a>
                @endif
                ...
                @if ($last > 0)
                    @for ($i = 1; $i <= min(5, $last); $i++)
                        ...
                        @else
                            <a href="{{ $fasilitas->url($i) }}" class="px-3 py-1.5 rounded-lg text-sm font-bold text-gray-600 hover:bg-[#B87C39]/10 hover:text-[#B87C39] transition">{{ $i }}</a>
                        @endif
                    @endfor

                    @if ($last > 5)
                        <span class="px-2 text-gray-400 text-sm">...</span>
                        <a href="{{ $fasilitas->url($last) }}" class="px-3 py-1.5 rounded-lg text-sm font-bold text-gray-600 hover:bg-[#B87C39]/10 hover:text-[#B87C39] transition">{{ $last }}</a>
                    @endif
                @endif

                @if ($fasilitas->hasMorePages())
                    <a href="{{ $fasilitas->nextPageUrl() }}" class="px-3 py-1.5 rounded-lg text-sm font-bold text-[#B87C39] hover:bg-[#B87C39]/10 transition">&rsaquo;</a>
                @else
            ...
            <div
                ...
                <form method="POST" :action="formAction" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="nama_fasilitas" name="nama_fasilitas" type="text" x-model="formNama" required class="w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-900 outline-none transition focus:border-[#B87C39] focus:ring-4 focus:ring-[#B87C39]/10">
                    </div>
